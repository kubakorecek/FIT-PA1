...
void insert_sorted_arch_by_type(TARCHIVE ** head_list,TENGINE ** engine)
{
        ...
        if(((*head_list)==NULL) || 
        //(*head_list)->m_Engines->m_Year > (*engine)->m_Year)
      ...
      while(current_arch->m_Next)
      {
        ...
        //if(current_arch->m_Next->m_Engines){
            
        //test =  strcmp(current_arch->m_Next->m_Engines->m_Type,(*engine)->m_Type);
        //}else{
         // test=-1;
       // }
      ...
      /*if(
        ...
        test = strcmp(current_arch->m_Engines->m_Type,(*engine)->m_Type);
        TARCHIVE * new_archive = initialize_archive_node();
        set_front_engine(&(new_archive->m_Engines),engine);
        push_front_archive(head_list,&new_archive);
        return;    
